...
1. Da ZAP andare su `Tools` dalla navbar in alto
2. Selezionare `Options` > `Network` > `Local Servers/Proxies`
3. Impostare come `Address` quello locale nella rete tramite menu a cascata e come `Port` una libera a scelta
...
1. Da ZAP andare su `Tools` dalla navbar in alto
2. Selezionare `Options` > `Network` > `Server Certificates`
3. Selezionare `Save`
...
1. Dal dispositivo andare in `Impostazioni` > `Sicurezza` > `Credenziali` > `Installa un certificato` > `CA Certificate` > `Install anyway` > Selezionare il certificate `.cer` importato precedentemente
2. Su alcune versioni di Android, potrebbe essere necessario abilitare il certificato come attendibile per reti Wi-Fi, questo si puo fare andando ad installare il medesimo certificato dalla scheda Wi-Fi delle impostazioni o dal `Sicurezza` > `Credenziali` e installare il certificato come certificato Wi-Fi
3. Controllare che il certificato sia stato accettato e installato tramite `Impostazioni` > `Sicurezza` > `Credenziali` > `Trusted credentials`  > `User` e controllare la presenza di `ZAP Root CA`
4. Spegnere il telefono chiudendo la finestra di Genymotion
5. Impostare nelle impostazioni di Genymotion il proxy HTTP con indirizzo `IP` e `porta` impostati su ZAP
6. Avviare il telefono
7. Da telefono acceso, dare questo comando da terminale per attivare il proxy e concludere il procedimento
...
- Le pagine protette da login non sono rilevabili senza configurazionedell'autenticazione di ZAP.
